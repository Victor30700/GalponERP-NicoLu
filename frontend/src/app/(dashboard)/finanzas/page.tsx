    ...
    <div className="space-y-8 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1 font-medium">Control de flujo de caja, gastos y proyecciones.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {kpis.map((kpi, idx) => (
          <motion.div
            ...
            className="p-6 glass-dark rounded-3xl border border-white/5"
          >
        ...
        {[
          ...
          <button
            ...
            className={cn(
              ...
              activeTab === tab.id ? "bg-primary text-black shadow-lg" : "text-slate-400 hover:text-white"
            )}
          >
            <tab.icon size={16} />
            {tab.label}
          ...
          <motion.div key="flujo" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="space-y-4">
                <h3 className="text-sm font-black text-white uppercase tracking-widest ml-2 flex items-center gap-2">
                  <ArrowUpRight className="text-emerald-500" size={18} />
                  Ingresos por Ventas
                </h3>
                <div className="glass-dark rounded-[2rem] border border-white/5 p-2">
                   <UniversalGrid
                    title="Ingresos"
                    ...
                    renderMobileCard={(item) => (
                      <div className="flex justify-between items-center">
                        <span className="font-bold text-white">Lote {item.lote}</span>
              ...
              <div className="space-y-4">
                ...
                <div className="glass-dark rounded-[2rem] border border-white/5 p-2">
                   <UniversalGrid
                    ...
                    renderMobileCard={(item) => (
                      <div className="flex justify-between items-center">
                        <span className="font-bold text-white truncate max-w-[150px]">{item.descripcion}</span>
          ...
          <motion.div key="gastos" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-1 space-y-4">
                <h3 className="text-sm font-black text-white uppercase tracking-widest ml-2">Gastos por Categoría</h3>
                <div className="space-y-3">
                  {gastosPorCategoria.map((cat, i) => (
                    <div key={i} className="p-4 glass-dark rounded-2xl border border-white/5 flex items-center justify-between">
              ...
              <div className="lg:col-span-2 space-y-4">
                <h3 className="text-sm font-black text-white uppercase tracking-widest ml-2">Detalle de Egresos</h3>
                <div className="glass-dark rounded-[2.5rem] border border-white/5 p-2">
                  <UniversalGrid
                    ...
                    columns={[
                      ...
                      { header: 'Descripción', accessor: 'descripcion' },
                      { header: 'Monto', accessor: (item) => <span className="font-black text-white">${item.monto.toLocaleString()}</span> }
                    ]}
                    renderMobileCard={(item) => (
                      <div className="space-y-2">
                        <div className="flex justify-between items-center">
                          <span className="text-[10px] font-black text-slate-500 uppercase">{item.tipoGasto}</span>
                          <span className="font-black text-white">${item.monto.toLocaleString()}</span>
                        </div>
          ...
          <motion.div key="proyeccion" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
               <div className="p-8 bg-gradient-to-br from-primary/20 to-transparent rounded-[2.5rem] border border-primary/10">
                  ...
                  <h2 className="text-4xl font-black text-white mb-4">${flujoProyectado?.flujoNetoProyectado30Dias.toLocaleString()}</h2>
               ...
               <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="p-6 glass-dark rounded-3xl border border-white/5 flex flex-col justify-between">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Costo Alimento (Proyectado)</p>
                    <p className="text-2xl font-black text-red-400">-${flujoProyectado?.costoProyectadoAlimento30Dias.toLocaleString()}</p>
                  </div>
                  <div className="p-6 glass-dark rounded-3xl border border-white/5 flex flex-col justify-between">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Cuentas por Cobrar</p>
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-black text-white uppercase tracking-widest ml-2">Línea de Tiempo de Proyecciones</h3>
              <div className="glass-dark rounded-[2.5rem] border border-white/5 p-2 overflow-hidden">
                <table className="w-full text-left">
                  <thead className="border-b border-white/5">
                    <tr>
                      <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase">Concepto</th>
                      <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase text-center">Tipo</th>
                      <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase">Fecha Estimada</th>
                      <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase text-right">Monto</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/5">
                    {flujoProyectado?.detalle.map((d, i) => (
                      <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                        <td className="px-6 py-4 text-xs font-bold text-white uppercase">{d.concepto}</td>
                        ...
                        <td className="px-6 py-4 text-xs text-slate-500 font-medium">{new Date(d.fechaEstimada).toLocaleDateString()}</td>
                        <td className="px-6 py-4 text-right font-black text-white tracking-tighter">${d.monto.toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
