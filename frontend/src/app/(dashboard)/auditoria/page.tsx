    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm">Trazabilidad completa de acciones críticas en el sistema.</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1 md:w-64">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-slate-900/50 border border-white/10 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all"
            />
          </div>
          <select 
            value={selectedEntidad}
            onChange={(e) => setSelectedEntidad(e.target.value)}
            className="px-4 py-2 bg-slate-900/50 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50 appearance-none"
          >
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            {filteredLogs.length === 0 ? (
              <div className="py-20 text-center text-slate-500 glass-dark rounded-2xl border border-white/5">
              ...
              <div className="grid grid-cols-1 gap-4">
                {filteredLogs.map((log, idx) => (
                  <motion.div
                    ...
                    className="glass-dark border border-white/5 p-4 rounded-2xl hover:bg-white/[0.02] transition-all group"
                  >
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="font-bold text-white">{log.usuarioNombre}</span>
                            <span className={cn(
                      ...
                      <div className="flex items-center gap-2 md:opacity-0 group-hover:opacity-100 transition-opacity self-end md:self-center">
                        <button 
                          ...
                          className="flex items-center gap-2 px-3 py-1.5 bg-white/5 hover:bg-white/10 text-slate-300 rounded-lg text-xs font-bold transition-all"
                        >
      ...
      <AnimatePresence>
        {selectedLog && (
          <>
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-2xl glass-dark z-[110] shadow-2xl rounded-3xl overflow-hidden">
              <div className="px-6 py-4 border-b border-white/10 bg-white/5 flex items-center justify-between">
                <h2 className="text-xl font-bold text-white flex items-center gap-2">
                  <FileJson className="text-indigo-400" />
                  Detalles Técnicos (JSON)
                </h2>
                <button onClick={() => setSelectedLog(null)} className="p-2 hover:bg-white/10 rounded-full text-slate-400 transition-all"><X size={20} /></button>
              </div>
              <div className="p-6 overflow-y-auto max-h-[70vh]">
                ...
                <pre className="p-4 bg-slate-950/50 rounded-2xl border border-white/5 text-indigo-300 font-mono text-xs overflow-x-auto">
